...
class UI extends THREE.Group {
  constructor() {
    ...
    this.scoreCv= document.createElement('canvas');
    this.scoreCtx=this.scoreCv.getContext('2d');

    this.scoreTex=new THREE.Texture(this.scoreCv);
    this.scoreTex.needsUpdate=true;
    this.scoreTex.anisotropy=8;
    var scoreGeo=new THREE.PlaneGeometry(.1, 0.05);
    var scoreMat= new THREE.MeshBasicMaterial({map: this.scoreTex, transparent: true, opacity: 0.75});
  ...
  updateCanvas(score) {
    this.scoreCtx.clearRect(0, 0, this.scoreCv.width, this.scoreCv.height);
    this.scoreCtx.fillColor = 'white';
    this.scoreCtx.fillText(score, this.scoreCv.width / 2, this.scoreCv.height / 2);
    this.scoreTex.needsUpdate = true;
  }
}
...
class Sprite extends THREE.Sprite {
  ...
  update(reserve) {
    if (!camera.down || simRunning || reserve == true) {
      this.ctx.clearRect(0, 0, this.cv.width, this.cv.height);
    } else {
      this.ctx.font = 24 + "pt CabinRegular";
      this.ctx.clearRect(0, 0, this.cv.width, this.cv.height);
      this.ctx.textAlign = 'center';
      this.ctx.textBaseline = 'middle';
      this.ctx.fillStyle = '#ffffff';
      this.ctx.fillText(this.value, this.cv.width / 2, this.cv.height / 2);
    }
    this.material.map.needsUpdate = true;

    this.position.copy(this.position).add(new THREE.Vector3(0, camera.down ? .2 : -1, 0));
  }
}
