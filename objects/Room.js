...
module.exports = class {
  ...
  addPlayer(name, socket) {
    ...
    this.number++;
    console.log('Player', name, 'added with socket', this.players[name].socket.id, 'in room', this.id);
  }

  resumePlayer(name, socket) {
    this.players[name].setSocket(socket);
    console.log('Player', name, 'resumed with socket', this.players[name].socket.id, 'in room', this.id);
  }
